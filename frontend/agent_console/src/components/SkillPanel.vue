...
const selected = computed(() => new Set(props.selection?.selected_skill ? [props.selection.selected_skill] : []));
</script>
...
<template>
  <div class="skill-panel">
    ...
    <div v-if="loadedSkill" class="loaded-skill">
      <BookOpenCheck :size="18" />
      <div><strong>{{ loadedSkill.name }}</strong><p>{{ loadedSkill.description }}</p><small>{{ loadedSkill.path }} · 约 {{ loadedSkill.estimated_tokens }} tokens</small></div>
    </div>
...
<style scoped>
.skill-panel, .skill-candidates { display: grid; gap: 10px; }
