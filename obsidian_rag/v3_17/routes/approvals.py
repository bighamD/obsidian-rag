...
from obsidian_rag.v3_15.schemas import ApprovalListResponse, ApprovalRecord, ApprovalResumeRequest
...
router = APIRouter(prefix="/approvals", tags=["deepagents-durable-hitl"])
...
def get_approval(run_id: str, service: DurableAgentLearningService = Depends(get_learning_service)):
...
@router.post("/{run_id}/resume", response_model=DurableAgentAskResponse)
def resume(
    run_id: str,
