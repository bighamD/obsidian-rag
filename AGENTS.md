...
- 使用稳定 `conversation_id -> thread_id` 和 PostgreSQL Checkpointer，实现跨请求、跨服务重启的同线程 messages 与 HITL 恢复。
- 使用 Conversation Repository 独立保存标题、scope、状态和 Thread 映射，不复制完整 Agent messages。
- 使用 LangGraph PostgresStore、`StoreBackend` 和 `(tenant_id, assistant_id, user_id)` namespace 保存跨线程长期 Memory。
- 使用 `CompositeBackend` 将 `/artifacts/`、`/context/` 和 `/memories/` 路由到 Run Sandbox、Thread Store 和 User Store。
- 使用 DeepAgents 模型 Profile 驱动的 Offloading/Summarization，不复刻固定 `memory_window` 或每四轮压缩规则。
- 提供 `remember_user_fact`、`list_user_memories`、`forget_user_memory`、Memory Policy、CRUD、Audit 和共享 Console 管理面板。
- 提供独立 `obsidian_rag/v3_17/`、FastAPI JSON/SSE、CLI、`launch.json`、Guide 和三张 SVG。

V3.17 仍然不做：

- 不把全部 Turn、完整回答、RAG chunks 或 Tool 原文自动写入长期 Memory。
- 不实现跨历史 Conversation 向量搜索、后台 Memory consolidation 或复杂 Sub-agent。
- 不声称 `durable_context` 调试投影就是供应商收到的精确 Wire Prompt。
