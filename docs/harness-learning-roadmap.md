...
V3.17 DeepAgents Durable Memory & Context：系统能持久恢复多轮消息并管理跨线程长期记忆（计划中）
V3.18 DeepAgents Production Customization：系统能通过 Middleware、Runtime Context 和 Sub-agent 接入业务（计划中）
...
建议版本：`V3.17 DeepAgents Durable Memory & Context`
...
V3.17 ：接入持久多轮会话、长期 Memory 和 Context 管理
