...
V3.12.1：AgentRuntime/Core Extraction，把成熟的 V3.8.1 Agent 内核和 V3.12 MCP Adapter 提升到无版本号公共运行时（计划中）
```
...
| MCP / 外部工具协议 | V3.12 | 已覆盖：stdio MCP Client、MCP Tool Adapter、Demo Server，以及把本地 RAG 暴露为 MCP Server。 |
...
| AgentRuntime / Core | V3.12.1 | 计划把 V3.8.1 的稳定 Agent、State、Context、Memory、Tool 执行契约和 V3.12 MCP Adapter 提升到无版本号公共核心；学习版本通过 Adapter 调用。 |
...
V3.12.1 AgentRuntime/Core Extraction：把 V3.8.1 完整执行内核和 V3.12 MCP Adapter 迁移为稳定公共核心，版本目录改为 Adapter/教学对照层
...
V3.12 已完成：使用官方 Python MCP SDK 和 `stdio` Transport，实现短生命周期 `ClientSession`、`initialize`、`tools/list`、`tools/call`、Tool/Result Adapter、超时和结果大小边界、Demo MCP Server 与只读 RAG MCP Server；提供独立 FastAPI、Swagger JSON、CLI、`launch.json`、测试代码、断点文档和 SVG。当前先显式调用 MCP Tool，不让 LLM 自动选择，也不实现 Permission 或 Sandbox。后续 V3.12.1 再完成公共 Agent Core 收敛，并把 MCP Adapter 接入统一 Tool Registry。
