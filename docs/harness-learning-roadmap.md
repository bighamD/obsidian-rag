...
从 V3.16 开始，主线从“继续扩展自研 Agent Core”切换为“使用官方 DeepAgents 搭建生产 Harness”。现有 `obsidian_rag/core/` 保留为原理对照组，不再优先增加新的通用 Planner/Executor 抽象。

接下来的生产实践路线：

```text
V3.16：DeepAgents Tool Loop & Artifact，跑通 search -> observation -> write -> approval -> artifact
V3.17：DeepAgents Durable Memory & Context，补持久多轮会话、长期记忆和上下文管理
V3.18：DeepAgents Production Customization，学习 Middleware、Runtime Context 和 Sub-agent
V3.19：Production Hardening & Takeover Drill，补观测、评测、可靠性、部署和真实业务迁移
```
...
Phase 13：DeepAgents Tool Loop & Artifact，迁移到官方 Harness 并跑通真实数据依赖
Phase 14：DeepAgents Durable Memory & Context，持久多轮会话、长期记忆和上下文压缩
Phase 15：DeepAgents Production Customization，Middleware、Runtime Context 和 Sub-agent
Phase 16：Production Hardening & Takeover Drill，生产观测、可靠性、部署和业务迁移
```
...
| Memory & State / 记忆状态 | V3.3、V3.5、V3.8、V3.8.1、V3.17 | 已有自研 AgentState、MySQL Raw Turns、最近窗口和滚动摘要；V3.17 将切换到 DeepAgents Checkpointer、StoreBackend 和 Summarization。 |
| Orchestrator / 任务编排 | V3.3、V3.4、V3.5、V3.6、V3.10.3、V3.16 | V3.3-V3.6 学习显式 node/edge；V3.10.3 学习高级 LangGraph；V3.16 学习 DeepAgents 的 messages/tool_calls 循环和 Middleware Harness。 |
...
| AgentRuntime / Core | V3.12.1、V3.12.4、V3.16 | 已提升自研 Agent 内核；V3.16 开始迁移到官方 `create_deep_agent`，旧 Core 保留为原理对照。 |
...
Permission -> Sandbox -> Artifact -> Checkpoint -> HITL -> Recovery
```

当前主要缺口已经不是某个孤立模块，而是自研执行模型与目标生产框架之间的差异：

```text
当前自研 Core：Planner 预先生成完整 Plan 和 Tool arguments，再由 Executor 顺序执行
目标 DeepAgents：LLM -> tool_call -> ToolMessage -> LLM，模型基于真实 Observation 再决定下一步
```

这一区别会直接影响“先检索，再根据检索结果生成文件”这类有真实数据依赖的任务。V3.16 不再继续给现有 `depends_on` 增加自研数据流语义，而是使用 DeepAgents 的迭代 Tool Loop 完成生产形态迁移。
...
V3.16 DeepAgents Tool Loop & Artifact：系统能基于 Tool Observation 动态生成后续写入内容（已完成）
V3.17 DeepAgents Durable Memory & Context：系统能持久恢复多轮消息并管理跨线程长期记忆（已完成）
V3.18 DeepAgents Production Customization：系统能通过 Middleware、Runtime Context 和 Sub-agent 接入业务（下一主线，计划中）
V3.19 Production Hardening & Takeover Drill：系统能按生产要求观测、评测、恢复和部署（计划中）
```
...
12. `V3.16` 不继续修补自研 Planner 数据流，而是把既有 Tool、Sandbox、HITL 和 Console 映射到 DeepAgents，先掌握真实 Tool Loop。
13. `V3.17` 在 Tool Loop 稳定后再引入 Checkpointer、StoreBackend 和 Summarization，避免同时学习执行循环与 Memory 生命周期。
14. `V3.18-V3.19` 再进入生产项目最常见的 Middleware、Runtime Context、Sub-agent、观测、评测和部署扩展点。
...
## Phase 13：DeepAgents Tool Loop & Artifact

建议版本：`V3.16 DeepAgents Tool Loop & Artifact`（已完成）

完成状态：V3.16 已使用官方 `deepagents.create_deep_agent` 跑通 Observation 驱动的 Markdown Artifact 链路；完整实现、Swagger payload、分支和断点见 [V3.16 Guide](v3-16-deepagents-tool-loop-guide.md)。

### 版本定位

V3.16 是从自研 Harness 迁移到官方 DeepAgents 的桥接版本。它不追求一次接入 DeepAgents 的全部能力，只解决一个当前架构无法可靠完成的生产问题：**后续 Tool arguments 必须基于前一个 Tool 的真实 Observation 动态生成**。
...
```text
User Message
  -> Deep Agent model call #1
  -> search_notes tool_call
  -> ToolMessage（知识库检索结果、chunk_id、来源）
  -> Deep Agent model call #2（读取真实 ToolMessage）
  -> write_file tool_call（content 基于检索结果生成）
  -> interrupt_on / HITL 审批
  -> Sandbox Backend 写入
  -> Artifact 登记
  -> Deep Agent model call #3
  -> 最终答案和下载链接
```

这里不要求固定生成四个 `PlanStep`。DeepAgents 可能先调用 `write_todos`，也可能在写入前读取文件；验收依据是业务 Tool 的因果顺序和数据来源，而不是计划步骤数量。
...
```text
Planner 一次性生成 Plan 和 arguments
  -> authorize_steps
  -> approval_gate
  -> execute_steps
```

当 Planner 生成 `write_file.arguments.content` 时，`search_notes` 尚未执行，因此文件正文可能来自模型先验知识。`depends_on` 当前只表达描述性依赖，没有 Observation 绑定和延迟参数生成语义。

DeepAgents 使用迭代式 Tool Loop：每次 Tool Result 都以 `ToolMessage` 回到 messages，下一次模型调用可以读取真实结果后再生成后续 Tool Call。这正是 V3.16 要学习的核心，不再优先自研通用 Dataflow Executor。

### 与现有版本的职责映射

| 自研能力 | DeepAgents 对应能力 | V3.16 处理方式 |
| --- | --- | --- |
| `Plan` / Planner JSON | `write_todos` + 模型逐轮决策 | 作为对照学习，不再作为执行真相来源 |
| `execute_steps` | Agent Tool Loop | 由 `create_deep_agent` 构建的 Graph 接管 |
| `ToolRegistry` | LangChain Tool / Tool Schema | 为 `search_notes` 编写稳定 Adapter |
| `StepResult` | `ToolMessage` / Tool Observation | 保留 chunk、source、score 等可观察字段 |
| `approval_gate` | `interrupt_on` / HITL Middleware | 对写入类 Tool 暂停并恢复 |
| Core Docker Sandbox | DeepAgents Backend | 通过 Backend Adapter 复用现有安全边界 |
| Artifact Store | Backend 写入后的 Artifact Adapter | 返回稳定下载元数据和 URL |
| 自研 EventBus | LangGraph/DeepAgents stream events | 适配为现有 `console.v1` SSE 契约 |

### 五个内部实施阶段（已完成）

V3.16 仍然只有一个版本目录，但实现和 Guide 必须按以下顺序展开，不能直接只展示最终大工程。

#### Stage 1：DeepAgents Anatomy

- 引入官方 `deepagents` 依赖并使用 `create_deep_agent` 创建最小 Agent。
- 检查返回的 `CompiledStateGraph`、默认 tools、默认 Middleware 和 State。
- 用无 Tool 的简单问题观察一次 model call、messages 输入和最终 AIMessage。
- 文档对比显式 `graph.add_node/add_edge` 与 DeepAgents Middleware Harness。

#### Stage 2：Read-only Search Tool Loop

- 将现有 Hybrid Retrieval/Reranking 封装为标准 `search_notes` Tool。
- Tool 参数只暴露业务必要字段，不把数据库连接、Principal 或内部服务对象交给模型。
- Tool Result 返回结构化检索 Observation，至少保留 `chunk_id`、source、content、score 和 collection。
- 验证 `LLM #1 -> search_notes -> ToolMessage -> LLM #2 -> answer`。

#### Stage 3：Observation-driven File Content

- 接入文件写入能力，但先聚焦数据是否正确流动。
- 第二次模型调用必须能在 messages 中看到 `search_notes` 的 ToolMessage。
- `write_file.arguments.content` 必须包含知识库事实和来源，不允许复用 Planner 阶段预制正文。
- 调试响应应展示业务 Tool Call 序列、ToolMessage 摘要和文件内容来源，但不得展示隐藏 chain-of-thought。

#### Stage 4：HITL、Sandbox 与 Artifact

- 写入类 Tool 配置 `interrupt_on` 或等价官方 HITL 能力。
- 审批前不产生最终文件；`deny` 不执行；`allow/edit` 从 Checkpoint 恢复。
- DeepAgents Backend Adapter 复用 V3.14 的 Docker、Workspace、路径和资源限制。
- 写入成功后登记 Artifact，返回文件名、MIME、size、hash 和下载 URL。
- 恢复或 SSE 重连不得重复写入同一个副作用 Tool。

#### Stage 5：FastAPI、SSE 与 Agent Console

- 提供 Swagger 可测试的 JSON ask 接口。
- 提供 SSE stream、审批查询和 resume 接口。
- 将 DeepAgents 的 model、tool、interrupt 和 terminal events 映射到现有 Console 契约。
- Console 实时展示 `search_notes -> write_file -> approval -> artifact`，不依赖请求结束后一次性补齐。
- 保留 CLI 和 `launch.json` 分阶段调试案例。

### 建议代码边界

```text
obsidian_rag/v3_16/
  app.py                 FastAPI 装配和 lifespan
  dependencies.py        Deep Agent、Tool、Backend、Checkpointer 装配
  agent.py               create_deep_agent 入口和运行适配
  schemas.py             API、Run、Approval、Artifact 对外契约
  service.py             JSON/SSE/Resume 应用服务
  tools/
    search_notes.py      现有 Retrieval -> LangChain Tool Adapter
  backends/
    sandbox.py           Core Sandbox -> DeepAgents Backend Adapter
  artifacts.py           Artifact 登记和下载映射
  routes/
    agent.py             ask/stream/resume
    artifacts.py         artifact list/download
```

具体文件可在实施时按 DeepAgents 当前 API 调整，但职责必须保持分层：Tool 不直接操作 FastAPI，Backend 不负责回答生成，Artifact 不承担 Checkpoint 职责。
...
GET  /agent/runs/{run_id}/approval
...
```text
V3.16 API server
V3.16 minimal Deep Agent
V3.16 search_notes Tool Loop
V3.16 recipe Markdown + approval
V3.16 resume approved artifact
```

### 完成标准

- 能从 Trace 中看到两次及以上 model call，而不是 Planner 一次性预制全部 Tool arguments。
- `search_notes` 的 ToolMessage 进入下一次模型调用的 messages。
- Markdown 正文包含检索命中的知识库事实、chunk_id 或来源标识。
- 审批前文件不存在，审批后只写入一次。
- 下载接口返回实际 Markdown 文件，Artifact 元数据与文件一致。
- JSON、SSE、CLI 和 Console 对同一 Run 的 Tool 顺序与终态一致。
- 能用断点解释 DeepAgents 默认 Middleware、model call、Tool execution、interrupt/resume 和 Backend 写入的执行顺序。

### 版本边界

- 不接入持久多轮对话和跨线程长期 Memory；这些进入 V3.17。
- 不接入 Skills、MCP、Sub-agent、动态 Model Router 或全部历史 Core Tool。
- 不继续增强自研 Planner/Executor 的 `depends_on` 数据流语义。
- Checkpointer 在本版本只服务 HITL/Recovery，不把它包装成完整 Conversation Memory。
- 不把隐藏 reasoning 或 chain-of-thought 暴露给 API 和前端。
- 测试代码按版本规范保留，但不采用 TDD，也不默认自动运行耗时测试。

### 文档与图解

V3.16 Guide 至少提供以下 SVG：

1. 自研 Planner/Executor 与 DeepAgents Tool Loop 对比图。
2. `search_notes ToolMessage -> next model call -> write_file` 数据流图。
3. `interrupt_on -> resume -> Sandbox -> Artifact` 时序图。

断点表必须按 Stage 1-5 分组，既给出本地 Adapter 断点，也说明如何进入 DeepAgents/LangGraph 源码观察框架内部调用。

## Phase 14：DeepAgents Durable Memory & Context

建议版本：`V3.17 DeepAgents Durable Memory & Context`（已完成）

完成状态：V3.17 已使用稳定 `conversation_id -> thread_id`、PostgreSQL Checkpointer、PostgresStore、`CompositeBackend`、`StoreBackend` 和 DeepAgents Summarization 跑通同 Thread 续聊、跨 Thread 长期 Memory、Context Offloading、CRUD/Audit 和共享 Console；完整实现与断点见 [V3.17 Guide](v3-17-deepagents-durable-memory-context-guide.md)。

### 版本定位

V3.17 对齐即将接手的生产任务：在 V3.16 已经稳定的 DeepAgents Tool Loop 上，实现持久多轮会话、跨线程用户记忆和 Context Window 管理。

核心状态分层：

| 状态 | 推荐承载 | 主要用途 |
| --- | --- | --- |
| 同线程 messages / Agent State | PostgreSQL Checkpointer | 同一个 `thread_id` 的多轮继续、恢复和 HITL |
| Conversation 列表、标题和业务状态 | Application Conversation Repository | 前端会话管理与业务查询 |
| 跨线程长期 Memory | LangGraph Store + `StoreBackend` | 用户偏好、长期事实和稳定决策 |
| Working files / scratch | `StateBackend` | 当前线程临时文件和上下文卸载 |
| 长上下文摘要 | DeepAgents `SummarizationMiddleware` | 接近模型 Context Window 时压缩旧消息 |

### 计划能力

- 使用 `thread_id` 恢复同一会话的 messages，不再手工固定读取最近 `memory_window` 条 Turn。
- 使用 `CompositeBackend` 将 `/memories/` 路由到持久 `StoreBackend`，其他工作文件保留在线程级 `StateBackend`。
- 使用 Runtime Context 传递 `tenant_id`、`user_id`、`assistant_id` 和鉴权信息。
- Memory namespace 默认至少包含 `(tenant_id, assistant_id, user_id)`，防止跨租户和跨用户串线。
- 区分短期 conversation memory、长期 semantic memory、episodic history 和 procedural skills。
- 使用 DeepAgents 默认 Offloading/Summarization 管理长会话；阈值遵循模型 profile，而不是固定四轮触发。
- 提供 Memory 查看、更新、删除、只读策略和 Audit 记录。
- 明确哪些信息可以长期保存：稳定偏好、长期事实、确认后的决策；临时问题、RAG chunk 和完整模型回答默认不写入长期 Memory。
- 保留 Conversation Repository，不把 Checkpoint 表直接当成前端会话业务表。

### 完成标准

- 相同 `thread_id` 在多次请求和服务重启后仍能继续对话。
- 新 `thread_id`、相同 `user_id` 可以读取已确认保存的长期偏好。
- 不同用户和租户之间的 Memory 完全隔离。
- Context 触发自动摘要后仍保留目标、关键决定、Artifact 和下一步信息。
- Swagger 和 Console 能区分 Thread History、Long-term Memory、Summary 与当前 Prompt Context。
- 能解释 Checkpointer、Store、Backend、Conversation Repository 和 Summarization 各自保存什么。

### 版本边界

- 不把每条对话原文自动复制为长期 Memory。
- 第一版不实现跨全部历史对话的向量语义搜索；需要时再通过独立 Tool 查询 episodic history。
- 第一版以 hot-path 明确写入为主；独立 consolidation agent 和 cron 可进入 V3.18。
- 不在 V3.17 同时引入复杂 Sub-agent 业务编排。

## Phase 15：DeepAgents Production Customization

建议版本：`V3.18 DeepAgents Production Customization`

目标：学习接手 DeepAgents 工程时最常见的定制点，而不是继续修改框架默认 Agent Loop。

计划覆盖：

- 自定义 Middleware、dynamic prompt 和 Tool 前后置 Hook。
- Runtime Context 中的 tenant、principal、feature flag、provider 和连接信息。
- 按用户和租户动态过滤 Tool、Memory、Skill 与 Backend scope。
- 自定义 Harness Profile、Tool description override 和 Model fallback。
- Sub-agent delegation 与 Context isolation。
- 对强顺序业务使用自定义 `CompiledStateGraph` Sub-agent，例如 `search -> compose -> validate -> approve -> write`。
- 独立 Memory consolidation agent，后台整理近期对话并合并长期记忆。

版本边界：Deep Agent 负责开放式决策；严格合规、事务或数据依赖流程放进确定性 Sub-agent，不要求通用 Agent Loop 充当任意 DAG 引擎。

## Phase 16：Production Hardening & Takeover Drill

建议版本：`V3.19 Production Hardening & Takeover Drill`

目标：用接近真实接手项目的方式完成一次生产变更，而不是再验证单个 Agent 概念。

计划覆盖：

- LangSmith tracing、dataset/evaluation、prompt/tool/model version。
- timeout、retry、cancellation、idempotency、SSE reconnect/replay。
- 多进程 Worker、Queue 或 LangGraph Platform 部署边界。
- Secrets、PII 脱敏、审计、租户隔离和 Memory 注入防护。
- Checkpoint/Store 迁移、备份、恢复和并发写冲突演练。
- 选择即将接手项目的一条真实业务链，按需求、设计、实现、观测、评测和生产 PR 流程完成迁移。
...
V3.16 ：迁移到 DeepAgents，跑通 Observation 驱动的 Markdown Artifact 链路（已完成）
V3.17 ：接入持久多轮会话、长期 Memory 和 Context 管理（已完成）
V3.18 ：学习 Middleware、Runtime Context、Sub-agent 和后台 Memory consolidation
V3.19 ：完成生产可靠性、观测、评测、部署和真实项目接手演练
```

不要把 V3.16-V3.19 合并为一次性大迁移。V3.16 只学习 DeepAgents Tool Loop 和 Artifact；V3.17 单独处理 Memory 生命周期；V3.18 才进入业务定制；V3.19 最后完成生产加固。现有 V3.10-V3.15 保留为原理对照和可复用 Adapter 来源。
