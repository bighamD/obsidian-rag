...
- Core `SkillResolver`：在 Planner 前发现、选择并按需加载一个 `SKILL.md`。
- `skill_selection` 和不含完整正文的 `loaded_skill` 摘要进入 JSON/SSE 与 Agent Console。
...
2. Skill Registry 只读取候选 manifests，LLM Skill Router 选择零个或一个 Skill。
3. 仅在选中后加载完整 `SKILL.md`，并把方法正文注入 Planner Context。
4. Planner 生成 `search`、`tool`、`synthesize` 等步骤；此时 Permission 尚未介入。
5. Collection Router 在存在 search step 时生成全局 `RetrievalScope`。
6. `authorize_steps` 对每个 PlanStep 生成 `PermissionDecision`。
7. `execute_steps` 只执行 `allow` 步骤。
8. `confirm` 步骤标记为 `skipped`，`deny` 步骤标记为 `failed`。
9. Evidence 和 Context 能观察权限结果，但不能绕过权限重新执行 Tool。
10. Answer 根据 `permission_report` 解释未执行的动作。
...
| `obsidian_rag/core/skills/router.py` | LLM Skill Router 结构化 JSON 选择。 |
| `obsidian_rag/core/skills/resolver.py` | 强制选择、关闭 Router、自动选择和加载的统一入口。 |
...
| 顺序 | 文件与函数 | 观察变量 |
| --- | --- | --- |
| S1 | `obsidian_rag/core/agent/service.py` `_discover_skills_node()` | `skill_candidates`、Registry errors。 |
| S2 | `obsidian_rag/core/agent/service.py` `_skill_router_node()` | `skill_selection`、`selected_skill`、confidence。 |
| S3 | `obsidian_rag/core/agent/service.py` `_load_skill_node()` | `loaded_skill`、`skill_context`、estimated tokens。 |

随后进入 `_planner_node()`，观察 Planner question 是否包含 `[Selected Skill Context]`。
