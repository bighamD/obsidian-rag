...
V3.12.1 : Version Adapters -> Agent Core -> Tool Registry -> JSON / answer_delta SSE
```
...
- 不展示 `reasoning_content`、chain-of-thought 或隐藏推理。
...
`answer_delta` 改善首字体验；完整 answer 仍用于 Memory、JSON 和最终对账，不会发起第二次 LLM 调用。
