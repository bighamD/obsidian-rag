...
> 当前代码说明：V3.12.3 首次引入的 Tool Catalog、统一 Tool 执行和
> `ToolObservation` 已在 V3.14 阶段提升到 `obsidian_rag/core/agent/service.py`。
> `McpAgentService` 现为兼容类名；本章保留能力演进背景。
...
| `obsidian_rag/v3_12_3/agent.py` | 保留 `McpAgentService` 兼容类名；通用 Tool 节点已进入 Core |
...
| 3 | `obsidian_rag/core/agent/service.py:600` | `_planner_node()` | `catalog`、`PlanRequest.tools`、`plan.steps` |
| 4 | `obsidian_rag/core/planner.py:212` | `_build_planner_messages()` | `user_payload["tools"]` |
| 5 | `obsidian_rag/core/agent/service.py:693` | `_execute_steps_node()` | `step.kind`、`step.tool_name` |
| 6 | `obsidian_rag/core/agent/service.py:737` | `_execute_tool_step()` | `step.arguments`、`ToolResult`、`ToolObservation` |
| 7 | `obsidian_rag/v3_12_3/connection.py:324` | `_call_in_session()` | `server.tools`、`result.isError`、`result_size` |
| 8 | `obsidian_rag/core/agent/service.py:867` | `_evidence_check_node()` | `failed_tools`、`evidence_check.is_sufficient` |
...
| 11 | `obsidian_rag/core/agent/service.py:1032` | `_synthesize_answer_node()` | `observations`、最终 Answer trace |
